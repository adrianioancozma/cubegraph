/-
  CubeGraph/GeneralProjection.lean
  ...
  CircuitProjection.lean. The general boolOr(non-perm, non-perm) case requires
  ...
  . ~20 theorems (gate_expr_good removed due to genuine gaps).
-/

import CubeGraph.EnrichedKR
import CubeGraph.CrossFanOut
...
/-! ## Parts 4-5: General Induction (removed)
  ...
  (1) boolOr(non-perm, non-perm): complementary sub-permutations can produce
      a permutation outside the sigma group. See BoolOrNonPerm.lean for the
  ...
  concrete instances in CircuitProjection.lean.
-/
