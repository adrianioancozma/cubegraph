/-
  ...
  DEPENDENCIES:
  - FinalGap.lean (exponential lower bound for rank-1)
  ...
  . Uses schoenebeck_linear (existing axiom) + frege axioms.
-/
...
/- REMOVED (2026-03-24): `resolution_cost_frege_superlinear` was a re-export of
...
/- REMOVED (2026-03-24): `creation_resolution_asymmetry_frege` depended on
   `frege_near_quadratic`, which depended on the unsound `frege_simulation` axiom.

   The theorem claimed Frege asymmetry: O(n) to create, Ω(n²/log n) to resolve.
   The rank-1 asymmetry (creation_resolution_asymmetry) remains valid. -/
...
/-- **Structural count**: 15 theorems in this file. -/
