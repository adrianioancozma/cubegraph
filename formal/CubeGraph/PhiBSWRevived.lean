/-
  CubeGraph/PhiBSWRevived.lean — Analysis of Sigma's "b/N constant" observation
  ...
  See: KappaFixedPoint.lean (BorromeanOrder preservation under ER)
...
/-- **EXPERIMENTAL AXIOM (Sigma, G5)**: BorromeanOrder scales proportionally
    with the number of cubes, even under ER extension.

    Formally: ∃ α_num α_den : Nat with α_num > 0, α_den > 0, such that
    for UNSAT G with BorromeanOrder b ≥ n/c, and any ER extension T(G):
    b(T(G)) ≥ α_num · |T(G).cubes| / α_den.

    STATUS: EXPERIMENTAL. Measured on formula-derived Tseitin extensions.
    NOT proven for proof-derived extensions. Kappa only gives b(T(G)) ≥ b(G). -/
axiom borromean_scales_proportionally :
    ∃ α_num α_den : Nat, α_num > 0 ∧ α_den > 0 ∧
      ∀ (G : CubeGraph),
        ¬ G.Satisfiable →
        ∀ (ext : ERExtension G),
          ...
          ¬ KConsistent ext.extended (α_num * ext.extended.cubes.length / α_den)
...
/-- **Conditional Frege bound with proportional BorromeanOrder.**

    IF borromean_scales_proportionally THEN:
    c · S · (c' · (|G| + c · S) + 1) ≥ (α · (|G| + c · S))² / α_den²

    This uses bsw_with_formula_size (the correct BSW with N denominator).
    The bound is WEAKER than FregeLowerBound.lean (gives Ω(n), not Ω(n²/log n)).

    The point: proportional scaling DOES NOT help. The BSW denominator
    absorbs the growth. -/
theorem conditional_frege_with_proportional_b :
    -- Using bsw_with_formula_size (correct BSW with denominator):
    -- From Schoenebeck + proportional scaling + Frege simulation:
    -- The bound we get is Ω(n), WEAKER than the existing Ω(n²/log n).
    -- We state the raw inequality from the chain (same as frege_near_quadratic).
    ∃ c₁ c₂ c₃ : Nat, c₁ ≥ 2 ∧ c₂ ≥ 1 ∧ c₃ ≥ 1 ∧ ∀ n ≥ 1,
      ∃ G : CubeGraph, G.cubes.length ≥ n ∧ ¬ G.Satisfiable ∧
        -- The existing near-quadratic bound (from FregeLowerBound.lean):
        c₂ * minFregeSize G * (c₃ * (G.cubes.length + c₂ * minFregeSize G) + 1) ≥
        (n / c₁) * (n / c₁) :=
  -- This is exactly frege_near_quadratic. The proportional BorromeanOrder
  -- axiom does NOT improve this bound (see Sections 2 and 6 for analysis).
...
/-- **WARNING**: Applying abd_bsw_resolution_exponential to extensions
    with k = Ω(N_ext) is UNSOUND. The axiom's constant c was calibrated
    for N = O(k · constant), which fails when N = k/α (proportional). -/
theorem bsw_axiom_misuse_warning :
    -- The axiom exists (trivially)
    (∃ c : Nat, c ≥ 2 ∧
      ∀ (G : CubeGraph) (k : Nat),
        KConsistent G k → ¬ G.Satisfiable →
        minResolutionSize G ≥ 2 ^ (k / c))
    -- But this does NOT mean: for extensions T(G) with k = Ω(|T(G)|),
    -- minRes T(G) ≥ 2^{Ω(|T(G)|)}.
    -- The axiom's c was computed under N = O(k·c_sch), not N = k/α.
    ∧ True := by
...
/-- The existing near-quadratic bound subsumes any benefit from
    proportional BorromeanOrder scaling. -/
theorem existing_bound_is_best :
    -- frege_near_quadratic gives Ω(n²/log n)
    -- proportional scaling would give only Ω(n) via bsw_with_formula_size
    -- Therefore: existing > proportional
    ∃ c₁ c₂ c₃ : Nat, c₁ ≥ 2 ∧ c₂ ≥ 1 ∧ c₃ ≥ 1 ∧ ∀ n ≥ 1,
      ∃ G : CubeGraph, G.cubes.length ≥ n ∧ ¬ G.Satisfiable ∧
        c₂ * minFregeSize G * (c₃ * (G.cubes.length + c₂ * minFregeSize G) + 1) ≥
        (n / c₁) * (n / c₁) :=
