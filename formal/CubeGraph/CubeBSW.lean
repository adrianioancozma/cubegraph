/-
  CubeGraph/CubeBSW.lean — Cube-BSW: Ben-Sasson-Wigderson on CubeGraph
  ...
  The denominator problem (BSWRevived.lean Section 7) is fundamental:
  ...
  See: BSWRevived.lean (detailed denominator analysis)
...
theorem cg_width_lower_bound_principle :
    -- ABD+BSW gives: k-consistency + UNSAT -> exponential Resolution size.
    -- NOTE (2026-03-24): Now includes M = G.cubes.length in the denominator,
    -- matching the actual BSW Corollary 3.6 bound.
    ∃ c : Nat, c ≥ 1 ∧
      ∀ (G : CubeGraph) (k : Nat),
...
/-- **CG-Resolution exponential lower bound.**
    Schoenebeck + ABD+BSW => Resolution size >= 2^{(n/c₁-2)²/(c₂·M)}.
    CubeGraph-native formulation: directly combines the two axioms
    using cube-counting.

    NOTE (2026-03-24): Previously claimed 2^{n/c} (without M denominator).
    Corrected to include M = G.cubes.length per the actual BSW Cor. 3.6.
    When M = O(n) (as for random 3-SAT), the exponent is still Omega(n). -/
theorem cubeBSW_resolution_exponential :
    ∃ c_k c_s : Nat, c_k ≥ 2 ∧ c_s ≥ 1 ∧ ∀ n ≥ 1,
      ∃ G : CubeGraph, G.cubes.length ≥ n ∧ ¬ G.Satisfiable ∧
        minResolutionSize G ≥
          2 ^ ((n / c_k - 2) * (n / c_k - 2) / (c_s * G.cubes.length)) := by
  ...
  exact ⟨c₁, c₂, hc₁, hc₂, fun n hn => by
    ...
    have hM : G.cubes.length ≥ 1 := Nat.le_trans hn hsize
    exact ⟨G, hsize, hunsat, h_abd G (n / c₁) hkc hunsat hM⟩⟩
...
/-! ## Section 5: ER Extension and the Denominator Problem
  ...
  This is EXACTLY the same situation as BSWRevived.lean Section 2.
...
/- REMOVED (2026-03-24): `cubeBSW_er_selfref` was a re-export of `frege_near_quadratic`,
   which depended on the unsound `frege_simulation` axiom.
   The observation that Cube-BSW gives the same bound as the Frege self-referential
   bound remains valid as commentary. -/
...
/-- **Summary of Cube-BSW contributions (sound parts only).** -/
theorem cubeBSW_summary :
    -- (1) Resolution exponential (CG-native, with M denominator)
    (∃ c_k c_s : Nat, c_k ≥ 2 ∧ c_s ≥ 1 ∧ ∀ n ≥ 1,
      ∃ G : CubeGraph, G.cubes.length ≥ n ∧ ¬ G.Satisfiable ∧
        minResolutionSize G ≥
          2 ^ ((n / c_k - 2) * (n / c_k - 2) / (c_s * G.cubes.length)))
...
/- NOTE: Component (3) cubeBSW_er_selfref (ER/Frege self-referential bound) was removed
   (2026-03-24) because it depended on unsound frege_simulation via frege_near_quadratic. -/
