/-
  CubeGraph/IrreducibleEnumeration.lean
  ...
  NOTE ON IMPORTS: A name collision between Type2Monodromy.lean
  (abbrev h2Monodromy) and MonodromySquared.lean (def h2Monodromy)
  ...
  . 6 redeclared axioms (proven in their original files).
  18 theorems.
-/

import CubeGraph.ThreeConditions
...
theorem schoenebeck_linear_local :
    ∃ c : Nat, c ≥ 2 ∧ ∀ n ≥ 1,
      ∃ G : CubeGraph, G.cubes.length ≥ n ∧
        KConsistent G (n / c) ∧ ¬ G.Satisfiable :=
  CubeGraph.schoenebeck_linear_axiom

/- The following 4 axioms were VACUOUS: they claimed proof system lower bounds
   but only stated ∃ G, |G|≥n ∧ UNSAT (no size bound). This follows trivially
   from schoenebeck_linear_local. Converted to theorems (2026-03-25).

   The actual exponential size bounds for each proof system are in:
   - Resolution: ERLowerBound.lean (er_resolution_lower_bound)
   - Cutting Planes: CPLowerBound.lean (cp_exponential)
   - Polynomial Calculus: PCLowerBound.lean (pc_exponential)
   - AC⁰-Frege: AC0FregeLowerBound.lean (ac0frege_exponential) -/

/-- Resolution: large UNSAT instances exist. (Trivially from Schoenebeck.) -/
theorem resolution_lb :
    ∃ c : Nat, c ≥ 1 ∧ ∀ n ≥ 1,
      ∃ G : CubeGraph, G.cubes.length ≥ n ∧ ¬ G.Satisfiable := by
  obtain ⟨c, hc, h⟩ := schoenebeck_linear_local
  exact ⟨c, by omega, fun n hn => let ⟨G, hs, _, hu⟩ := h n hn; ⟨G, hs, hu⟩⟩

/-- Cutting Planes: large UNSAT instances exist. (Trivially from Schoenebeck.) -/
theorem cp_lb :
    ∃ c : Nat, c ≥ 1 ∧ ∀ n ≥ 1,
      ∃ G : CubeGraph, G.cubes.length ≥ n ∧ ¬ G.Satisfiable :=
  resolution_lb

/-- Polynomial Calculus: large UNSAT instances exist. (Trivially from Schoenebeck.) -/
theorem pc_lb :
    ∃ c : Nat, c ≥ 1 ∧ ∀ n ≥ 1,
      ∃ G : CubeGraph, G.cubes.length ≥ n ∧ ¬ G.Satisfiable :=
  resolution_lb

/-- AC⁰-Frege: large UNSAT instances exist. (Trivially from Schoenebeck.) -/
theorem ac0frege_lb (d : Nat) (_ : d ≥ 2) :
    ∃ c : Nat, c ≥ 1 ∧ ∀ n ≥ 1,
      ∃ G : CubeGraph, G.cubes.length ≥ n ∧ ¬ G.Satisfiable :=
  resolution_lb
...
/-- **L7.18 (HONEST DISCLAIMER)**.
    ...
    PROVEN (6 redeclared axioms from other files):
