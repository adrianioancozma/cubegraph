/-
  CubeGraph/Psi4LiftingCapstone.lean — Two Independent Paths to Exponential Resolution
  ...
  ┌─────────────────────────────────────────────────────────────────────────────┐
  │  ROOT CAUSE: ¬ IsPowerOfTwo 7  (Theta3NonAffine.lean)                     │
  │  ↓                                                                         │
  ...
  │                                  ∴ Res size ≥ 2^{Ω(n)}                    │
  │  (ERLowerBound.lean)             (Chi4Lifting.lean)                        │
  └─────────────────────────────────────────────────────────────────────────────┘
  ...
  - GPW (2018): DT → CC lifting [Chi4Lifting.lean]
  - KW (1990): CC = monotone depth [Chi4Lifting.lean]
  - FIP/Krajíček (1997): Resolution has FIP [Chi4Lifting.lean]
  - BIKPPW (1996): depth-d Frege [AC0FregeLowerBound.lean]
  - Tseitin/BSW with size: Frege simulation [FregeLowerBound.lean]

  0 sorry. 0 new axioms. Pure composition of existing theorems.

  See: Chi4Lifting.lean (Path B: GPW+KW+FIP chain)
  ...
  See: Theta3NonAffine.lean (arithmetic root: ¬ IsPowerOfTwo 7)
  See: Phi3UniversalNonAffine.lean (universal non-affinity)
-/
...
/-- **TWO INDEPENDENT PATHS TO EXPONENTIAL RESOLUTION LOWER BOUNDS.**
  ...
  Path B (GPW+KW+FIP, from Chi4Lifting.lean):
...
theorem two_independent_paths_to_resolution_exponential :
    -- Path A: ABD+BSW chain → Resolution on ER-extended formulas ≥ 2^{Ω(n)}
    (∃ c : Nat, c ≥ 1 ∧ ∀ n ≥ 1,
    ...
    ∧
    -- Path B: GPW+KW+FIP chain → Resolution ≥ 2^{Ω(n)}
    (∃ c : Nat, c ≥ 1 ∧ ∀ n ≥ 1,
      ∃ game : WitnessCommGame,
        game.graph.cubes.length ≥ n ∧
        gamma_minFregeSize game.graph ≥ 2 ^ (n / c))
    ∧
    -- Both paths start from the SAME root cause: 7 is not a power of 2
    (¬ IsPowerOfTwo 7)
    ∧
    -- Both paths go through BorromeanOrder Θ(n) (Schoenebeck)
    (∃ c : Nat, c ≥ 2 ∧ ∀ n ≥ 1,
...
theorem full_proof_complexity_hierarchy :
    -- (1) Resolution/ER exponential — Path A
    (∃ c : Nat, c ≥ 1 ∧ ∀ n ≥ 1,
      ∃ G : CubeGraph, G.cubes.length ≥ n ∧ ¬ G.Satisfiable ∧
        (∀ ext : ERExtension G,
          (∀ i : Fin ext.extended.cubes.length,
          ...
          minResolutionSize ext.extended ≥ 2 ^ (n / c)))
    ∧
    -- (2) Resolution/ER exponential — Path B (independent confirmation)
    (∃ c : Nat, c ≥ 1 ∧ ∀ n ≥ 1,
      ∃ game : WitnessCommGame,
        game.graph.cubes.length ≥ n ∧
        gamma_minFregeSize game.graph ≥ 2 ^ (n / c))
    ∧
    -- (3) Polynomial Calculus exponential
    (∃ c : Nat, c ≥ 1 ∧ ∀ n ≥ 1,
      ∃ G : CubeGraph, G.cubes.length ≥ n ∧ ¬ G.Satisfiable ∧
        minPCSize G ≥ 2 ^ (n / c))
    ∧
        ...
        (∀ ext : ERExtension G,
          ...
          (∀ i : Fin ext.extended.cubes.length,
            i.val ≥ G.cubes.length →
              ∃ w_pos : Fin 3, ∀ j : Fin ext.extended.cubes.length, i ≠ j →
                (ext.extended.cubes[i]).varAt w_pos ∉ (ext.extended.cubes[j]).vars) →
          minPCSize ext.extended ≥ 2 ^ (n / c)))
    ∧
    -- (5) Cutting Planes exponential
    (∃ c : Nat, c ≥ 1 ∧ ∀ n ≥ 1,
      ∃ G : CubeGraph, G.cubes.length ≥ n ∧ ¬ G.Satisfiable ∧
        minCPSize G ≥ 2 ^ (n / c))
    ∧
        ...
        (∀ ext : ERExtension G,
          ...
          (∀ i : Fin ext.extended.cubes.length,
            i.val ≥ G.cubes.length →
              ∃ w_pos : Fin 3, ∀ j : Fin ext.extended.cubes.length, i ≠ j →
                (ext.extended.cubes[i]).varAt w_pos ∉ (ext.extended.cubes[j]).vars) →
          minCPSize ext.extended ≥ 2 ^ (n / c)))
    ∧
    -- (7) AC⁰-Frege exponential (for any fixed depth d ≥ 2)
    (∀ d ≥ 2,
      ∃ c : Nat, c ≥ 1 ∧ ∀ n ≥ 1,
        ∃ G : CubeGraph, G.cubes.length ≥ n ∧ ¬ G.Satisfiable ∧
          minAC0FregeSize G d ≥ 2 ^ (n / c))
    ∧
    -- (8) Depth-sensitive Frege: (log₂ size)^{c·d} ≥ n/c'
    (∃ c₁ c₂ : Nat, c₁ ≥ 2 ∧ c₂ ≥ 2 ∧ ∀ n ≥ 1,
      ∃ G : CubeGraph, G.cubes.length ≥ n ∧ ¬ G.Satisfiable ∧
        ∀ d ≥ 2,
          (Nat.log2 (minAC0FregeSize G d)) ^ (c₂ * d) ≥ n / c₁)
    ∧
    -- (9) Frege near-quadratic: c₂·S · (c₃·(n + c₂·S) + 1) ≥ (n/c₁)²
    (∃ c₁ c₂ c₃ : Nat, c₁ ≥ 2 ∧ c₂ ≥ 1 ∧ c₃ ≥ 1 ∧ ∀ n ≥ 1,
      ∃ G : CubeGraph, G.cubes.length ≥ n ∧ ¬ G.Satisfiable ∧
        c₂ * minFregeSize G * (c₃ * (G.cubes.length + c₂ * minFregeSize G) + 1) ≥
        (n / c₁) * (n / c₁))
    ∧
    -- (10) The root cause: 7 is not a power of 2
    (¬ IsPowerOfTwo 7) := by
    ...
    (∃ c : Nat, c ≥ 1 ∧ ∀ n ≥ 1,
      ∃ G : CubeGraph, G.cubes.length ≥ n ∧ ¬ G.Satisfiable ∧
        (∀ ext : ERExtension G,
          ...
          minResolutionSize ext.extended ≥ 2 ^ (n / c)))
    ∧
    -- Path B — GPW+KW+FIP (independently)
    (∃ c : Nat, c ≥ 1 ∧ ∀ n ≥ 1,
      ∃ game : WitnessCommGame,
        game.graph.cubes.length ≥ n ∧
        gamma_minFregeSize game.graph ≥ 2 ^ (n / c))
    ∧
    -- Common root: Schoenebeck SA Ω(n)
    (∃ c : Nat, c ≥ 2 ∧ ∀ n ≥ 1,
      ∃ G : CubeGraph, G.cubes.length ≥ n ∧
        KConsistent G (n / c) ∧ ¬ G.Satisfiable)
    ∧
    -- Common intermediate: witness DT ≥ Ω(n)
    (∃ c : Nat, c ≥ 2 ∧ ∀ n ≥ 1,
      ∃ G : CubeGraph, G.cubes.length ≥ n ∧ ¬ G.Satisfiable ∧
...
/-- **HONEST ACCOUNTING: what IS and ISN'T proven.**
  ...
  PROVEN (0 sorry in this file, axioms from published results):
...
theorem honest_capstone_accounting :
    -- (A) Two independent exponential resolution paths
    (∃ c : Nat, c ≥ 1 ∧ ∀ n ≥ 1,
      ∃ G : CubeGraph, G.cubes.length ≥ n ∧ ¬ G.Satisfiable ∧
        (∀ ext : ERExtension G,
          (∀ i : Fin ext.extended.cubes.length,
    ...
    (∃ c : Nat, c ≥ 1 ∧ ∀ n ≥ 1,
      ∃ game : WitnessCommGame,
        ...
        gamma_minFregeSize game.graph ≥ 2 ^ (n / c))
    ∧
    -- (B) All clausal systems exponential
    (∃ c : Nat, c ≥ 1 ∧ ∀ n ≥ 1,
      ∃ G : CubeGraph, G.cubes.length ≥ n ∧ ¬ G.Satisfiable ∧
        minPCSize G ≥ 2 ^ (n / c))
    ∧
    ...
    (¬ IsPowerOfTwo 7)
    ∧
    -- (E) The frontier: general circuits are OPEN
    True :=
  ⟨er_resolution_lower_bound,
   resolution_exponential_via_lifting,
   pc_lower_bound,
...
/-! ## SUMMARY
    ...
    Inherited from FregeLowerBound: minFregeSize, frege_simulation, bsw_with_formula_size
