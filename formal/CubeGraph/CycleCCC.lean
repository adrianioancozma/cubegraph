/-
  ...
  CENTRAL INSIGHT:
    ...
    - XOR-SAT: O(d) bits suffice (d = cycle basis dimension).
      ...
      Bob computes the answer. (CycleBasis.lean)

    - 3-SAT: O(d) bits do NOT suffice.
      Boolean trace is NOT additive (CycleBasis.lean).
  ...
  FORMALIZED RESULTS (22 theorems):
  ...
  See: CycleBasis.lean (XOR additivity, OR non-additivity)
  See: UniversalWordProblem.lean (single vs universal word)
  See: CCLifting.lean (GPW lifting for witness CC)
  See: KRConsequences.lean (Z/2Z in T3*, KR >= 1)
  See: Monodromy.lean (monodromy diagonal iff CycleFeasibleAt)
-/

import CubeGraph.CycleBasis
import CubeGraph.UniversalWordProblem
