/-
  CubeGraph/Sigma4SevenSteps.lean
  ...
  reduce to rank-1 composition — which IS the P vs NP problem (Xi4PNeNP.lean).

  DEPENDENCIES:
  - Eta4Orthogonality.lean (orthogonality theorem, all four pillars)
  - Phi3UniversalNonAffine.lean (universal non-affinity: p^d - 1 ≠ p^k)
  - TaylorBarrier.lean (no WNU3 preserves ≠ on Fin 3)

  0 sorry. Uses schoenebeck + schoenebeck_linear (existing axioms).
-/
...
/-- **THE 7-STEP CHAIN**: From arithmetic (7 ≠ 2^k) to the exponential
    ...
    PROTOCOLS only. It does NOT prove P ≠ NP. See Xi4PNeNP.lean. -/
...
/-- **Theorem count**: 11 theorems in this file, 0 sorry. -/
