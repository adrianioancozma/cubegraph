/-
  CubeGraph/FixedPointChain.lean — Fixed-Point Argument for ER Exponential
  ...
  . 0 new axioms. All proofs derive from existing infrastructure.
-/
...
/-- **ER Exponential via Fixed Point** (synthesis of existing results).
    ...
    ALL intermediate results are PROVEN.
...
theorem er_exponential_via_fixed_point :
    ...
    ∀ n ≥ 1,
      ∃ G : CubeGraph, G.cubes.length ≥ n ∧ ¬ G.Satisfiable ∧
        (∀ ext : ERExtension G,
          ...
          -- Resolution on T(G): size >= 2^{(n/c_k - 2)²/(c_s·M_ext)}
          -- NOTE (2026-03-24): includes M_ext denominator per BSW Cor. 3.6
  ...
  exact ⟨hunsat_ext, hkc_ext, fun hM_ext =>
    h_abd ext.extended (n / c₁) hkc_ext hunsat_ext hM_ext⟩
...
theorem bsw_denominator_warning :
    -- The ABD+BSW result (with M denominator, corrected 2026-03-24):
    (∃ c : Nat, c ≥ 1 ∧
      ∀ (G : CubeGraph) (k : Nat),
        KConsistent G k → ¬ G.Satisfiable → G.cubes.length ≥ 1 →
        minResolutionSize G ≥
          2 ^ ((k - 2) * (k - 2) / (c * G.cubes.length)))
    -- Applied to extended formulas (with M_ext denominator):
...
/-- **Fixed Point Summary.**
    ...
    PROVEN (0 new axioms, all from existing infrastructure):
...
theorem fixed_point_summary :
    ...
    -- (2) ER exponential (with M_ext denominator, corrected 2026-03-24)
    ∧ (∃ c_k c_s : Nat, c_k ≥ 2 ∧ c_s ≥ 1 ∧ ∀ n ≥ 1,
        ∃ G : CubeGraph, G.cubes.length ≥ n ∧ ¬ G.Satisfiable ∧
          (∀ ext : ERExtension G,
            (∀ i : Fin ext.extended.cubes.length,
