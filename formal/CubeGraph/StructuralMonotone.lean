/-
  CubeGraph/StructuralMonotone.lean
  ...
  See: CCLifting.lean (GPW lifting chain)
-/
...
/-- The CubeGraph gap consistency interpolant IS monotone.
    Direct corollary of gapConsistency_mono from GapConsistency.lean. -/
...
/- REMOVED (2026-03-24): `random_3sat_no_algebraic_structure` axiom deleted.
   Category D dead code — zero downstream uses outside this file.
   Tautological: it asserted ∃ G with MonotoneInterpolant G, but
   `cubegraph_interpolant_monotone` already proves MonotoneInterpolant G
   for ALL CubeGraphs unconditionally. The axiom added no information. -/

/-- Random 3-SAT CubeGraphs universally avoid BPR.
    (No longer needs the removed axiom — uses alpha_schoenebeck_linear for
     existence of large G, and cubegraph_interpolant_monotone for monotonicity.) -/
theorem random_3sat_avoids_bpr :
    ∀ n ≥ 1,
      ∃ G : CubeGraph, G.cubes.length ≥ n ∧
        ¬ RequiresNonMonotone G := by
  intro n hn
  obtain ⟨c, _, hsch⟩ := alpha_schoenebeck_linear
  obtain ⟨G, hsize, _, _⟩ := hsch n hn
  exact ⟨G, hsize, bpr_not_applicable G (cubegraph_interpolant_monotone G)⟩
...
/-! ## SUMMARY
  ...
  AXIOMS (0 new, inherited from AlphaGapConsistency):
  (random_3sat_no_algebraic_structure REMOVED 2026-03-24 — tautological dead code)
  (factoring_requires_nonmonotone was already converted to trivial theorem)
