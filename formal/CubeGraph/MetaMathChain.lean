/-
  ...
  Status: 0 new axioms (uses existing schoenebeck_linear only).
  ...
  Note: AffineComposition.lean (affine/P direction) cannot be imported
...
/-! ## Part IV: The Dichotomy — Affine vs Non-Affine
  ...
  XOR-SAT preserves rank) is proved in AffineComposition.lean.
...
/-- **The Dichotomy Theorem**: affine (P) vs non-affine (NP).
    ...
    The CONTRAST (from AffineComposition.lean, cited):
...
/-- **THE META-MATHEMATICAL CHAIN** — the complete 8-step argument.
    ...
    - NonAffine.lean         (Steps 1-2: arithmetic + geometry)
    - IrreversibleDecay.lean (Steps 3-5: algebra)
    - BandSemigroup.lean            (Step 4: matrix aperiodicity)
    - AffineComposition.lean  (Dichotomy: affine direction, cited)
    - InformationBottleneckComplete.lean (Step 6: information)
    - SchoenebeckChain.lean         (Step 7: Borromean scaling)
    - FinalGap.lean         (Step 8: exponential bound)

    AXIOMS USED: schoenebeck_linear (Schoenebeck 2008, well-established).
    NO new axioms introduced in this file.
...
/-- **The Honest Gap**: the meta-logical distance from our theorem to P ≠ NP.
    ...
    WHAT WE PROVED (1 axiom from Schoenebeck 2008):
...
/-- **Classes eliminated by the meta-mathematical chain.**
    ...
    5. C_local (boolean composition) — local consistency barrier
...
theorem chain_classes_eliminated :
    ...
    -- C_local barrier: locally consistent + UNSAT
