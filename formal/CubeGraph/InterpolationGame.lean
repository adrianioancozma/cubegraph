/-
  CubeGraph/InterpolationGame.lean — Feasible Interpolation for CubeGraph
  ...
  THIS FILE PROVES (2 axioms citing published results):
  ...
  See: GapConsistency.lean (monotone circuit LB via Razborov)
...
/- Monotone interpolant lower bound (orphaned doc — axiom removed).
...
-- REMOVED (2026-03-29 audit): monotone_interpolant_exponential — overclaimed.
-- Claims Razborov 1985 gives exponential monotone interpolant lb for CG gap
-- consistency. But Razborov's bound is specific to CLIQUE, not gap consistency.
-- The extension to gap consistency requires NEW function-specific analysis
-- that has not been done. See AXIOM-AUDIT.md

/-! ## Section 5: Main Theorem — FIP implies exponential proof size -/

/- COMMENTED OUT (2026-03-29 audit): fip_implies_exponential
   Depended on monotone_interpolant_exponential which was removed (overclaimed).
...
/- COMMENTED OUT (2026-03-29 audit): resolution_exponential_via_fip
   Depended on fip_implies_exponential which was removed.
...
/- COMMENTED OUT (2026-03-29 audit): frege_fip_implies_exponential
   Depended on fip_implies_exponential which was removed.
...
/-! ## HONEST ACCOUNTING
    ...
    - monotone_interpolant_exponential: CONJECTURE (overclaims Razborov)
