/-
  ...
  THE CHAIN (all steps Lean-proven):
  ┌─────────────────────────────────────────────────────────────────────┐
  │  7 ≠ 2^k       (NonAffine.lean)                             │
  ...
  DEPENDENCIES:
  - IrreversibleDecay.lean (irreversible_rank_decay_bool, synthesis)
  - NonAffine.lean (7 non-affine, non-power-of-2)
  - (AffineComposition.lean contains rank contrast computations, not imported
...
/-- **THE FINAL GAP THEOREM**: Irreversible rank decay → exponential cost.
    ...
    Resolution lower bounds are in CubeBSW.lean (via BSW axiom). -/
...
/-! ## Section 7: The Contrast — Why XOR-SAT Escapes
  ...
  Full rank contrast computations are in AffineComposition.lean:
...
/-! ## Section 8: What Remains Open
  ...
  (c) Show Frege proofs cannot polynomially simulate beyond rank-1
      → Partial: CubeBSW.lean (resolution size), EFLowerBound.lean
