/-
  ...
  See: GapConsistency.lean (monotone circuit LB, gapConsistency_mono)
  See: InterpolationGame.lean (FIP framework, Craig interpolation)
  See: StructuralMonotone.lean (structural monotonicity, BPR non-applicability)
  See: NoCancellation.lean (OR/AND semiring has no cancellation)
  See: GeometricReduction.lean (geometric_reduction, tripartite_equivalence)
-/

import CubeGraph.InterpolationGame
import CubeGraph.StructuralMonotone
...
/-- Evidence item 1: Gap consistency is monotone (PROVEN).
...
/-- Evidence item 2: BPR non-applicability (PROVEN).
...
/-- Evidence item 3: Transfer operators are monotone in gap masks (PROVEN).
...
/-- Evidence item 4: Boolean matrix multiplication is monotone (PROVEN).
...
/-! ## Part 3: FIP from Restriction Monotonicity
  ...
  3. This is exactly fip_implies_exponential from InterpolationGame.lean -/
...
/- **Cook's Theorem + Cook-Reckhow — AXIOM REMOVED (overclaimed).**
...
/- REMOVED (2026-03-25): `frege_exponential_implies_no_poly_sat` overclaimed Cook-Reckhow.

   The axiom claimed: Frege exponential → no polynomial SAT algorithm.
   This is FALSE: Frege being exponential does NOT imply P!=NP.
   Extended Frege (or other proof systems) could still be polynomial.
   Cook-Reckhow (1979) shows P=NP iff there exists a polynomially bounded
   proof system — but that system need not be Frege.

   The axiom was unused (no theorem depended on it). -/
...
/-! ## Part 5: The Complete Chain Assembly
  ...
  All intermediate steps are PROVEN. -/
...
/-! ## Part 8: Honest Accounting
  ...
  2. (REMOVED) frege_exponential_implies_no_poly_sat — overclaimed Cook-Reckhow
  ...
  All intermediate steps are machine-verified.
