/-
  CubeGraph/Gamma2WeakExpander.lean — Weak Expander Bridge to Bounded-Depth Frege
  ...
  See: OmicronKR.lean (rank2_kr_geq_one — Z/2Z witness)
...
/-- **GT24 Expansion Axiom**: Random 3-CNF at linear density produces
...
axiom random_cubegraph_weak_expander :
    ∃ c₀ : Nat, c₀ ≥ 1 ∧ ∀ n ≥ 10,
      ∃ (G : CubeGraph) (numVars : Nat),
        numVars ≥ n ∧
        G.cubes.length ≥ n ∧
        ∃ we : WeakExpander (toBipartite G numVars),
          we.r ≥ n / c₀ ∧ we.delta = 3 ∧ we.c ≥ 1
...
/-- **GT24 Lemma 14**: Weak expansion forces large resolution width.
...
axiom weak_expander_resolution_width :
    ∀ (G : CubeGraph) (numVars : Nat) (we : WeakExpander (toBipartite G numVars)),
      minResolutionWidth G ≥ we.c * we.r / 2

/-! ## Section 5: GT24 Super-Linear Bound -/

/-- **Gryaznov-Talebanfard Main Theorem (2024)**: For every depth k,
...
/-! ## Section 6: The Rank-1 Bottleneck (Proven, 0 sorry)
...
- Rank-2 operators generate Z/2Z: KR >= 1 (OmicronKR.lean)
...
/-- **Rank-1 Algebraic Bottleneck**: rank-1 is aperiodic (KR = 0) and closed,
    ...
    All components proven in existing files with 0 sorry. -/
...
/-- **Summary**: the three pillars connecting GT24 to CubeGraph.

    PILLAR 1 (GT24): Weak expansion => width >= Omega(n) => super-linear Frege.
    PILLAR 2 (CubeGraph algebra): Rank-1 aperiodic (KR=0), rank-2 has KR>=1.
    PILLAR 3 (Schoenebeck+CubeGraph): k-consistency at Omega(n) on UNSAT.

    All three see the SAME underlying phenomenon: information cannot flow
    efficiently through the CubeGraph structure.

    GT24 quantifies this as "width" (syntax).
    Schoenebeck as "k-consistency" (semantics).
    CubeGraph as "rank bottleneck" (algebra).

    The unification is: width = k-consistency level = rank stabilization depth.
    All three are Omega(n). All three give the same lower bound structure.
    None achieves super-polynomial by itself. -/
theorem three_pillar_summary :
    -- Pillar 1: GT24 (axiom)
    (∀ k : Nat, k ≥ 1 → ∃ n₀, ∀ n ≥ n₀,
      ∀ G : CubeGraph, G.cubes.length ≥ n → ¬ G.Satisfiable →
        minAC0FregeSize G k ≥ n) ∧
    -- Pillar 2: rank-1 aperiodic + rank-2 KR >= 1 (proven)
    ((∀ M : BoolMat 8, M.IsRankOne → mul M (mul M M) = mul M M) ∧
     (∃ (g e : BoolMat 8), IsZ2Group g e ∧ g ≠ e)) ∧
    -- Pillar 3: Schoenebeck (axiom) + concrete witness (proven)
    ((∃ c, c ≥ 2 ∧ ∀ n ≥ 1, ∃ G : CubeGraph, G.cubes.length ≥ n ∧
        KConsistent G (n / c) ∧ ¬ G.Satisfiable) ∧
     (∃ G : CubeGraph, KConsistent G 2 ∧ ¬ KConsistent G 3 ∧ ¬ G.Satisfiable)) := by
  refine ⟨gt24_superlinear_bound, ⟨?_, ?_⟩, ⟨schoenebeck_linear, ?_⟩⟩
  · intro M hM; exact rank1_aperiodic hM
  · exact rank2_kr_geq_one
  · exact ⟨h2Graph, h2graph_2consistent, h2graph_not_3consistent, h2Graph_unsat⟩
