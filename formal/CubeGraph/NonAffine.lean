/-
  ...
  See: experiments-ml/031_2026-03-26_matrix-proof-system/DEEP-THINKING-NEW-IDEAS.md (F3: bool_or_no_additive_inverse)
...
/-! ## Section 9: Boolean Semiring ≠ Field (F3)

  The boolean semiring ({false, true}, OR, AND) is NOT a field.
  Specifically: the multiplicative identity (true) has NO additive inverse.
  That is, there is no x : Bool such that (true || x) = false.

  This is the algebraic reason OR-based matrices cannot "cancel" information:
  unlike GF(2) where every element has an additive inverse (XOR), the boolean
  semiring is irreversible — once OR sets a bit to true, it cannot be undone.

  Contrast with GF(2): true ⊕ true = false (has inverse). -/

/-- F3: The boolean semiring does NOT satisfy additive inverse for 1.
    There is no x : Bool such that (true OR x) = false.
    Proved by exhaustive case check (native_decide). -/
theorem bool_or_no_additive_inverse :
    ¬ ∃ x : Bool, (true || x) = false := by
  native_decide

/-- F3 consequence: The boolean semiring ({false, true}, OR, AND) is NOT a field.
    A field requires additive inverses; the boolean semiring lacks them for 1. -/
theorem bool_semiring_not_field :
    ¬ (∀ a : Bool, ∃ b : Bool, (a || b) = false) := by
  intro h
  have := h true
  exact bool_or_no_additive_inverse this
