/-
  CubeGraph/WLLifting.lean — WL Lifting Analysis (de Rezende et al. STOC 2025)
  ...
  WHAT THIS FILE PROVIDES (all honest):
  ...
  - BSWRevived.lean (BSW denominator barrier analysis).
  - WeakExpander.lean (GT24 + WL connection).

  See: agents/2026-03-21-UPSILON2-WL-LIFTING.md (full analysis)
-/

import CubeGraph.WeakExpander
...
/- REMOVED (2026-03-25): de_rezende_width_depth_tradeoff was DEAD CODE — declared
   but never used in any proof. The conditional_depth_from_wl theorem takes
   the trade-off as a hypothesis instead.

   It stated: de Rezende Theorem 1.4 (Width-Depth, STOC 2025).
   ∀ (C : Nat), ∃ n₀ : Nat, ∀ n ≥ n₀, ∃ (G : CubeGraph),
     G.cubes.length ≥ n ∧ ¬ G.Satisfiable ∧
     minResolutionDepth G (n / 4 + C) ≥ n

   NOTE: Applied to compressed Tseitin formulas, NOT random 3-SAT.
   Reference: de Rezende et al. STOC 2025, arXiv:2411.14267. -/
...
/- REMOVED (2026-03-25): de_rezende_treelike_supercritical was DEAD CODE —
   declared but never used in any proof. The treelike bound avoids the BSW
   denominator but only for treelike resolution (Frege simulates treelike).

   It stated: de Rezende Theorem 1.3 (Width-Size Treelike, STOC 2025).
   ∃ n₀ : Nat, ∀ n ≥ n₀, ∃ (G : CubeGraph),
     G.cubes.length ≥ n ∧ ¬ G.Satisfiable ∧
     minTreelikeSize G (n / 3) ≥ 2 ^ n

   Reference: de Rezende et al. STOC 2025, arXiv:2411.14267. -/

/-! ## Section 5: The BSW Denominator Barrier (Proven, from BSWRevived.lean)
...
This is the same conclusion as BSWRevived.lean (Section 7). -/

/- REMOVED (2026-03-24): `bsw_denominator_persists` was a re-export of
   `frege_near_quadratic`, which depended on the unsound `frege_simulation` axiom.

   The analysis conclusion remains: de Rezende's results do not provide a
...
theorem wl_confirms_schoenebeck :
    ...
    -- Rank-1 algebraic bottleneck (proven)
...
- bsw_denominator_persists (REMOVED — unsound frege_simulation dependency)
