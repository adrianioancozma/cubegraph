/-
  CubeGraph/GammaWitnessProperties.lean — Witness Function Properties from BorromeanOrder
  ...
  HONEST ACCOUNTING:
  - Theorems 1-7: PROVEN (from KConsistent + BorromeanOrder, 0 sorry)
...
/-! ## Section 0: Re-declarations from upstream files that don't build

    These are IDENTICAL copies of definitions/axioms from:
    - KConsistency.lean: KConsistent, sat_implies_kconsistent
    - InformationChannel.lean: BorromeanOrder, blind_below_borromean
    - SchoenebeckChain.lean: schoenebeck_linear
    - FregeLowerBound.lean: minFregeSize
    - WitnessReduction.lean: minWitnessCircuit, frege_to_witness

    Once the upstream files are fixed for Lean 4.29, this section should be
    replaced by proper imports. -/
...
/-- Minimum Frege proof size for UNSAT CubeGraph (axiom). -/
axiom gamma_minFregeSize (G : CubeGraph) : Nat

/-- Minimum circuit size for the witness function (axiom). -/
axiom gamma_minWitnessCircuit (G : CubeGraph) : Nat

/-- Frege proof size >= witness circuit size (axiom, folklore). -/
axiom gamma_frege_to_witness :
...
/-! ## SUMMARY
    ...
    AXIOMS USED (3, all re-declared locally due to build issues):
    - gamma_schoenebeck_linear: Schoenebeck (2008) — SA level Omega(n)
    - gamma_minFregeSize, gamma_minWitnessCircuit: circuit/proof size specifications
